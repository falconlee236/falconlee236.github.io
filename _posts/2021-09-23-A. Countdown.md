...
## 문제 설명  
당신은 N자리 정수를 나타낼 수 있는 전자 시계를 가지고 있다. 각 자리는 0부터 9까지 정수를 표현할 수 있다. 그래서 모든 시계는 0부터 10^N - 1까지의 수를 표현한다. 이 시계는 만약 숫자가 10^(N - 1)보다 작으면 0으로 시작할 수 있다.  
<br>
...
당신의 목표는 위에 적힌 지시사항을 최소로 사용해서 시계가 0이 나타나게 하는 것이다. 

__Input__  
이 문제는 여러개의 테스트 케이스로 이루어져 있다. 첫번째 줄에는 테스트 케이스의 개수 t가 주어진다. (1 <= t <= 10^3)
